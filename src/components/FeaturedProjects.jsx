...
const ProjectCard = ({ project, index }) => {
  return (
    ...
    >
      <div className="relative w-full aspect-video overflow-hidden rounded-2xl bg-slate-100 shadow-sm border border-slate-200/50">
        <a href={project.url !== "#" ? project.url : undefined} target={project.url !== "#" ? "_blank" : "_self"} rel="noreferrer" className={`block w-full h-full ${project.url === "#" ? "cursor-default" : "cursor-pointer"}`}>
      ...
      <div className="flex flex-col gap-3">
        <div className="flex flex-col md:flex-row md:justify-between md:items-baseline gap-1">
          <h3 className="text-2xl font-semibold text-slate-900 tracking-tight group-hover:text-blue-600 transition-colors duration-300">
            {project.title}
          </h3>
        ...
        {project.links && (
           <div className="flex flex-wrap gap-3 mt-2">
              {project.links.map((link, idx) => (
                <a key={idx} href={link.url} target="_blank" rel="noreferrer" className="inline-flex items-center gap-1.5 text-sm font-medium text-slate-700 hover:text-blue-600 transition-colors bg-slate-100 hover:bg-blue-50 px-3 py-1.5 rounded-full">
...
const FeaturedProjects = () => {
  return (
    <section className="bg-white text-slate-900 py-32 px-6 md:px-12 border-t border-slate-100">
